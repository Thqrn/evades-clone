...
function loadSecondary() {
    ...
    game.worlds[6] = new World(
        new Vector(0, 267),
        6,
        transformingTurbidityImpossible
    );
...
function animate(time) {
    ...
    if (isActive) {
        ...
        if (fps.length >= 10)
            averageFPS =
                fps.map((i) => (x += i), (x = 0)).reverse()[0] / fps.length;
    } else {
        fps = [];
    }
    ...
    if (!inMenu) {
        ...
        var area =
            game.worlds[game.players[0].world].areas[game.players[0].area];
        var wasVictory = area.getActiveBoundary().t;
        var strokeColor = "#425a6d";
        if (area.title_stroke_color) {
            strokeColor = area.title_stroke_color;
        }
        area = wasVictory
            ? "Victory!"
            : game.worlds[game.players[0].world].areas[game.players[0].area]
        ...
        context.textAlign = "center";
        context.strokeText(
            game.worlds[game.players[0].world].name + ": " + area,
            width / 2,
            40
        );
        context.fillText(
        ...
        if (settings.timer) {
            const timerTime = secondsFormat(
                Math.floor(game.players[0].timer / 1000)
            );
            context.beginPath();
            context.textAlign = "center";
            context.lineWidth = 6;
            ...
            context.strokeStyle = strokeColor; //"#425a6d";
            context.font =
                "bold " + 35 + "px Tahoma, Verdana, Segoe, sans-serif";
            context.textAlign = "center";
        ...
        if (
            game.worlds[game.players[0].world].areas[game.players[0].area].text
        ) {
            const text =
                game.worlds[game.players[0].world].areas[game.players[0].area]
                    .text;
            context.beginPath();
            context.font =
                "bold " + 25 + "px Tahoma, Verdana, Segoe, sans-serif";
            context.textAlign = "center";
            (context.lineWidth = 5),
                (context.strokeStyle = "#006b2c"),
                (context.fillStyle = "#00ff6b"),
        ...
        if (
            ...
            game.players[0].area == 1 &&
            game.players[0].world == 3
        ) {
            context.beginPath();
            context.font =
                "bold " + 35 + "px Tahoma, Verdana, Segoe, sans-serif";
            ...
            (context.lineWidth = 5),
                ...
                (context.fillStyle = "#00ff6b"),
                context.strokeText(
                    "this map has 120 areas... make sure you have time :D",
                    width / 2,
                    height - 120
                );
            context.fillText(
        ...
        if (world.selectedIndex == 3 && !loaded) {
            ...
            (context.lineWidth = 5),
                ...
                (context.fillStyle = "#00ff6b"),
                context.strokeText(
                    "this is to import a map, top left in the menu",
                    width / 2,
                    height - 120
                );
            context.fillText(
...
if (!window.localStorage.hat) {
    window.localStorage.hat = "Gold";
}
document.getElementById("wreath").value = window.localStorage.hat;
