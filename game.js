...
class Game {
    ...
    update(time) {
        ...
        for (var i in this.players) {
            this.players[i].update(
                time,
                this.worlds[this.players[i].world].friction,
                this.worlds[this.players[i].world].magnet
            );
            if (!this.players[i].wallGod) this.teleport(this.players[i]);
            if (!this.players[i].wallGod)
                this.worlds[this.players[i].world].collisionPlayer(
                    this.players[i].area,
                    this.players[i]
                );
            ...
            for (var j in loaded[i]) {
                if (loaded[i][j]) {
                    ...
                    for (var k in this.players) {
                        if (
                            this.players[k].world == i &&
                            this.players[k].area == j
                        ) {
    ...
    teleport(player) {
        ...
        for (var i in area.zones) {
            if (area.zones[i].type == 5) {
                var pos1 = new Vector(
                    this.worlds[player.world].pos.x +
                        area.pos.x +
                        area.zones[i].pos.x,
                    this.worlds[player.world].pos.y +
                        area.pos.y +
                        area.zones[i].pos.y
                );
                var pos2 = new Vector(
                ...
                var teleporter = closestPointToRectangle(
                    player.pos,
                    pos1,
                    pos2
                );
                ...
                if (dist < player.radius && !player.onTele) {
                    ...
                    var maxArea = 0;
                    var targetPoint = new Vector(
                        player.pos.x + area.zones[i].translate.x,
                        player.pos.y + area.zones[i].translate.y
                    );
                    for (var j in this.worlds[player.world].areas) {
                        var rect =
                            this.worlds[player.world].areas[j].getBoundary();
                        ...
                        var closest = closestPointToRectangle(
                            targetPoint,
                            new Vector(rect.x, rect.y),
                            new Vector(rect.w, rect.h)
                        );
    ...
    getStates(index) {
        ...
        obj.text = area.text;
        obj.pos = new Vector(
            area.pos.x + this.worlds[player.world].pos.x,
            area.pos.y + this.worlds[player.world].pos.y
        );
        ...
        for (var i = 0; i < areas.length; i++) {
            ...
            for (var j = 0; j < zones.length; j++) {
                ...
                if (zones[j].type == "teleport" || zones[j].type == "exit") {
                    block.translate = new Vector(
                        zones[j].translate.x / 32,
                        zones[j].translate.y / 32
                    );
                }
                ...
                for (var k in spawner) {
                    ...
                    if (object.type == "wind" || object.type == "wind_ghost") {
                        object.ignore_invulnerability =
                            values.ignore_invulnerability;
                    }
...
class Area {
    ...
    update(time, players, worldPos) {
            ...
            for (var j in this.entities[i]) {
                ...
                for (var v in this.assets) {
                    if (this.assets[v].type == 1) {
                        var rect = {
                            x: this.assets[v].pos.x,
                            y: this.assets[v].pos.y,
                            w: this.assets[v].size.x,
                            h: this.assets[v].size.y,
                            t: false,
                            wall: true,
                        };
                ...
                if (this.eng == 3) {
                    for (var v in this.zones) {
                        if (this.zones[v].type == 1) {
                            var rect = {
                                x: this.zones[v].pos.x,
                                y: this.zones[v].pos.y,
                                w: this.zones[v].size.x,
                                h: this.zones[v].size.y,
                                t: false,
                                wall: true,
                            };
                            this.entities[i][j].colide(rect);
        ...
        for (var i in this.entities) {
            newEntities[i] = [];
            for (var j in this.entities[i]) {
                var inside = pointInRectangle(
                    this.entities[i][j].pos,
                    {
                ...
                if (!inside && this.entities[i][j].weak) {
                } else {
                    newEntities[i].push(this.entities[i][j]);
                }
            }
        ...
        for (var i in this.entities) {
            for (var j in this.entities[i]) {
                if (
                    this.entities[i][j].collide &&
                    !this.entities[i][j].no_collide
                ) {
                    var fixed = closestPointToRectangle(
                        this.entities[i][j].pos,
                        {
                            x: boundary.x + this.entities[i][j].radius,
                            y: boundary.y + this.entities[i][j].radius,
        ...
        for (var i in players) {
            players[i].abilities(time, this, {
                x: this.pos.x + worldPos.x,
                y: this.pos.y + worldPos.y,
            });
            for (var j in this.entities) {
                for (var k in this.entities[j]) {
                    this.entities[j][k].interact(
                        players[i],
                        {
                            x: this.pos.x + worldPos.x,
                            y: this.pos.y + worldPos.y,
    ...
    collisionPlayer(player, worldPos) {
        ...
        var fixed = closestPointToRectangle(
            player.pos,
            new Vector(boundary.x + player.radius, boundary.y + player.radius),
            new Vector(
                boundary.w - player.radius * 2,
                boundary.h - player.radius * 2
            )
        );
        if (
            Math.abs(fixed.x - player.pos.x) +
                Math.abs(fixed.y - player.pos.y) !==
            0
        ) {
                ...
                if (
                    ...
                    if (Math.abs(relX) > Math.abs(relY)) {
                        ...
                        if (relX > 0) {
                            // Right collision
                            player.pos.x =
                                rectCenterX + rectHalfSizeX + player.radius;
                            player.vel.x = 0;
                        } else {
                            // Left collision
                            player.pos.x =
                        ...
                        if (relY < 0) {
                            // Up collision
                            player.pos.y =
                                rectCenterY - rectHalfSizeY - player.radius;
                            player.vel.y = 0;
                        } else {
                            // Bottom collision
                            player.pos.y =
    ...
    load() {
        ...
        for (var i in this.preset) {
            let pattern_amount = this.pattern_amount
                ? this.pattern_amount.slice()
                : NaN;
            do {
                ...
                var count = Math.round(this.preset[i].count * countMultiplier);
                var radius = this.preset[i].radius * radiusMultiplier;
                var speed = this.preset[i].speed * speedMultiplier;
                var x = this.preset[i].x;
                var y = this.preset[i].y;
                var angle = undefined;

                if (
                    typeof this.preset[i].angle === "string" &&
                ...
                if (typeof radius === "string" && currentVariables.length > 0) {
                    if (radius.startsWith("var")) {
                        radius = find_variable(
                            this.preset[i].radius,
                            currentVariables,
                            hashVariables,
                            pattern_id,
                            pattern_amount
                ...
                if (
                    typeof auraRadius === "string" &&
                    currentVariables.length > 0
                ) {
                    if (auraRadius.startsWith("var")) {
                        auraRadius = find_variable(
                            this.preset[i].auraRadius,
                            currentVariables,
                            hashVariables,
                            pattern_id,
                            pattern_amount
                ...
                for (var j = 0; j < count; j++) {
                    var posX =
                        Math.random() * (boundary.w - radius / 16) +
                        boundary.x +
                        radius / 32;
                    var posY =
                        Math.random() * (boundary.h - radius / 16) +
                        boundary.y +
                        radius / 32;
                    if (x !== undefined) {
                        var posX = x / 32;
                    ...
                    var rand = Math.floor(
                        Math.random() * this.preset[i].type.length
                    );
                    var enemy = new Unknown(
                    ...
                    if (this.preset[i].type[rand] == "wall") {
                        enemy = new Wall(
                            ...
                            radius / 32,
                            speed,
                            this.getActiveBoundary(),
                            j,
                            count,
                            this.preset[i].move_clockwise
                        );
                    }
                    ...
                    if (this.preset[i].type[rand] == "frost_giant") {
                        enemy = new FrostGiant(
                            new Vector(posX, posY),
                            radius / 32,
                            speed ? speed : 0,
                            angle,
                            this.preset[i].direction,
                            this.preset[i].turn_speed,
                            this.preset[i].shot_interval,
                            this.preset[i].cone_angle,
                            this.preset[i].pause_interval,
                            this.preset[i].pause_duration,
                            this.preset[i].turn_acceleration,
                            this.preset[i].shot_acceleration,
                            this.preset[i].pattern,
                            this.preset[i].immune,
                            this.preset[i].projectile_duration,
                            this.preset[i].projectile_radius,
                            this.preset[i].projectile_speed,
                            this.preset[i].precise_movement
                        );
                    }
                    if (this.preset[i].type[rand] == "speed_sniper") {
                        enemy = new SpeedSniper(
                            new Vector(posX, posY),
                            radius / 32,
                            speed,
                            angle
                    ...
                    if (this.preset[i].type[rand] == "magnetic_nullification") {
                        enemy = new MagneticNullification(
                            new Vector(posX, posY),
                            radius / 32,
                            speed,
                            angle,
                    ...
                    if (
                        this.preset[i].type[rand] == "positive_magnetic_sniper"
                    ) {
                        enemy = new PositiveMagneticSniper(
                            new Vector(posX, posY),
                            radius / 32,
                            speed,
                            angle
                    ...
                    if (
                        this.preset[i].type[rand] == "negative_magnetic_sniper"
                    ) {
                        enemy = new NegativeMagneticSniper(
                            new Vector(posX, posY),
                            radius / 32,
                            speed,
                            angle
                    ...
                    if (
                        this.preset[i].type[rand] == "wind" ||
                        this.preset[i].type[rand] == "wind_ghost"
                    ) {
                        enemy = new Wind(
                            new Vector(posX, posY),
                            radius / 32,
                            speed,
                            angle,
                            this.preset[i].ignore_invulnerability
                        );
                    }
                    ...
                    if (
                        this.preset[i].type[rand] == "positive_magnetic_ghost"
                    ) {
                        enemy = new PositiveMagneticGhost(
                            new Vector(posX, posY),
                            radius / 32,
                            speed,
                            angle
                    ...
                    if (
                        this.preset[i].type[rand] == "negative_magnetic_ghost"
                    ) {
                        enemy = new NegativeMagneticGhost(
                            new Vector(posX, posY),
                            radius / 32,
                            speed,
                            angle
                    ...
                    if (this.preset[i].type[rand] == "flower") {
                        enemy = new Flower(
                            ...
                            speed,
                            angle,
                            this.preset[i].growth_multiplayer
                        );
                    }
    ...
    addSniperBullet(type, pos, angle, radius, speed, duration = 4000, spawner) {
        switch (type) {
            ...
            case 1:
                ...
                var bullet = new IceSniperBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 2:
                ...
                var bullet = new RadiatingBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 3:
                ...
                var bullet = new SpeedSniperBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 4:
                ...
                var bullet = new RegenSniperBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 5:
                ...
                var bullet = new CorrosiveSniperBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 6:
                ...
                var bullet = new PoisonSniperBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 7:
                ...
                var bullet = new PositiveMagneticSniperBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 8:
                ...
                var bullet = new NegativeMagneticSniperBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 9:
                ...
                var bullet = new frost_giant_ice_bullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed,
            ...
            case 10:
                ...
                var bullet = new leaf_projectile(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 11:
                ...
                var bullet = new StickySniperBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
            ...
            case 13:
                ...
                var bullet = new WindSniperBullet(
                    new Vector(pos.x, pos.y),
                    angle,
                    radius,
                    speed
