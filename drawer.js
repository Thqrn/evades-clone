...
function renderArea(area, players, focus, old) {
    ...
    (ligth.width = width), (ligth.height = height);
    var world = game.worlds[players[0].world];
    if (
        old.area != players[0].area ||
        old.world != players[0].world ||
        (!canv && tiles.complete)
    ) {
    ...
    if (canv) {
        context.drawImage(
            canv.can,
            -focus.x * fov + width / 2 + area.pos.x * fov,
            -focus.y * fov + height / 2 + area.pos.y * fov
        ); //-focus.x*fov,-focus.y*fov)//(-focus.x+world.pos.x)*fov,(-focus.y+world.pos.y)*fov);
    }
    ...
    for (var i in players) {
        try {
            ...
            var grad = context1.createRadialGradient(
                width / 2 + (player.pos.x - focus.x) * fov,
                height / 2 + (player.pos.y - focus.y) * fov,
                0,
                width / 2 + (player.pos.x - focus.x) * fov,
                height / 2 + (player.pos.y - focus.y) * fov,
                (player.radius + 30 / 32) * fov
            );
        ...
        if (player.flashlight_active) {
            player.energy -= 1 / 30;
            if (player.energy <= 0) {
                player.flashlight_active = false;
            }
            try {
                var player = players[i];
                var grad = context1.createRadialGradient(
                ...
                if (player.inputAng < 0) {
                    player.inputAng += 360;
                }
                if (player.inputAng >= 360) {
                    player.inputAng -= 360;
                }
                var distanceOne = player.inputAng - Math.abs(player.lastAng);
                if (
                    player.lastAng <= player.inputAng + rotationSpeed &&
                    player.lastAng >= player.inputAng - rotationSpeed
                ) {
                } else if (distanceOne < -180) {
                    player.lastAng += rotationSpeed;
                } else if (distanceOne > 180) {
                    player.lastAng -= rotationSpeed;
                ...
                if (
                    ...
                    player.lastAng = player.inputAng;
                }
                ...
                context1.arc(
                    width / 2 + (player.pos.x - focus.x) * fov,
                    height / 2 + (player.pos.y - focus.y) * fov,
                    (flashlight_distance / 32) * fov,
                    (Math.PI / 180) * (-flashlight_angle + player.lastAng),
                    (Math.PI / 180) * (flashlight_angle + player.lastAng)
                );
                context1.fill();
                context1.closePath();
    ...
    for (let i in area.entities) {
        for (let j in area.entities[i]) {
            ...
            if (ent.isLight) {
                try {
                    var grad1 = context1.createRadialGradient(
                        width / 2 + (area.pos.x + ent.pos.x - focus.x) * fov,
                        height / 2 + (area.pos.y + ent.pos.y - focus.y) * fov,
                        0,
                        width / 2 + (area.pos.x + ent.pos.x - focus.x) * fov,
                        height / 2 + (area.pos.y + ent.pos.y - focus.y) * fov,
                        (ent.radius + ent.lightCount / 32) * fov
                    );
    ...
    for (var i in area.assets) {
        ...
        if (zone.type == 6 || zone.type == 8 || zone.type == 4) {
            try {
                ...
                var grad1 = context1.createRadialGradient(
                    width / 2 + (area.pos.x + zone.pos.x - focus.x) * fov,
                    height / 2 + (area.pos.y + zone.pos.y - focus.y) * fov,
                    0,
                    width / 2 + (area.pos.x + zone.pos.x - focus.x) * fov,
                    height / 2 + (area.pos.y + zone.pos.y - focus.y) * fov,
                    (lightPower / 32) * fov
    ...
    const devStat =
        "Comb Spd: " + combineSpeed(player) + ", Amount: " + player.safeAmount;
    let offset = 0;
    if (diff == "Easy") {
        const deathCounter = settings.dev
            ? "Death Count: " + players[0].deathCounter + ", " + devStat
            : "Death Count: " + players[0].deathCounter;
        context.fillText(deathCounter, 0, 20);
        // context.strokeText(deathCounter, 0, 20);
        const fpsCounter = settings.dev
            ? "FPS: " + averageFPS
            : "FPS:" + averageFPS;
        context.fillText(fpsCounter, 0, 45);
        // context.strokeText(fpsCounter, 0, 45);
    } else if (diff == "Medium") {
        const lives = settings.dev
            ? "Lives: " + players[0].lives + ", " + devStat
    ...
    if (settings.dev) {
        context.fillStyle = "gray";
        context.strokeStyle = "gray";
        const safePoint = player.safePoint
            ? "Safe Point: {X:" +
              Math.round(player.safePoint.pos.x * fov) +
              ", Y: " +
              Math.round(player.safePoint.pos.y * fov) +
              "} ([), to clear (])"
            : "None ([)";
        ...
        const playerPos =
            "Player: {X:" +
            Math.round(player.pos.x * fov) +
            ", Y: " +
            Math.round(player.pos.y * fov) +
            ", Speed: " +
            greaterMax(player) +
            "}";
...
function renderTiles(area, players, focus) {
    ...
    let wid = boundary.w * 32,
        heig = boundary.h * 32,
        world = game.worlds[players[0].world];
    var tile_image = tiles;
    const can = createOffscreenCanvas(wid, heig);
    const ctx = can.getContext("2d");
    ctx.translate(
        -Math.round(width / 2 + area.pos.x * fov),
        -Math.round(height / 2 + area.pos.y * fov)
    );
    for (var i in area.zones) {
        ...
        if (area.pos.x != Math.round(area.pos.x))
            areaXMinus = Math.round(area.pos.x) - area.pos.x;
        if (area.pos.y != Math.round(area.pos.y))
            areaYMinus = Math.round(area.pos.y) - area.pos.y;
        for (var j = 0; j < zone.size.x; j++) {
            for (var k = 0; k < zone.size.y; k++) {
                ...
                ctx.drawImage(
                    tile_image,
                    Math.abs(posX) * 32 + textureType * 128,
                    Math.abs(posY) * 32 + area.texture * 128,
                    32,
                    32,
                    Math.round(width / 2 + (area.pos.x + zone.pos.x + j) * fov),
                    Math.round(
                        height / 2 + (area.pos.y + zone.pos.y + k) * fov
                    ),
                    fov,
                    fov
                );
                ...
                if (zone.color) {
                    ...
                    ctx.fillRect(
                        Math.round(
                            width / 2 + (area.pos.x + zone.pos.x + j) * fov
                        ),
                        Math.round(
                            height / 2 + (area.pos.y + zone.pos.y + k) * fov
                        ),
        ...
        for (var j in entities[i]) {
            if (entities[i][j].renderFirst) {
                if (i == "shield") {
                    ...
                    context.translate(
                        width / 2 +
                            (area.pos.x + entities[i][j].pos.x - focus.x) * fov,
                        height / 2 +
                            (area.pos.y + entities[i][j].pos.y - focus.y) * fov
                    );
                    context.rotate(entities[i][j].rot);
                    context.beginPath();
                    context.fillStyle = "black";
                    context.fillRect(
                        -entities[i][j].size.x * fov,
                        -entities[i][j].size.y * fov,
                        entities[i][j].size.x * fov * 2,
                        entities[i][j].size.y * fov * 2
                    );
    ...
    for (var i in players) {
        ...
        if (player.aura) {
            if (player.auraType == 0) {
                ...
                context.arc(
                    width / 2 + (player.pos.x - focus.x) * fov,
                    height / 2 + (player.pos.y - focus.y) * fov,
                    ((100 + Math.abs(greaterMax(player)) * 5) / 32) * fov,
                    0,
                    Math.PI * 2,
                    true
                );
        ...
        if (!player.die) {
            if (player.wallGod) {
                ...
                if (player.night) {
                    context.fillStyle = `rgb(${rgb[0]},${rgb[1]},${rgb[2]},0.6)`;
                }
                if (player.mortarTime > 0 && player.mortarTime < 1000) {
                    context.fillStyle = `rgb(${rgb[0]},${rgb[1]},${rgb[2]},${
                        1 - player.mortarTime / 1000
                    })`;
                }
                if (player.fusion) {
                    context.fillStyle = "rgba(60, 60, 75)";
                }
            }
        } else {
            ...
            if (player.shape > 0) {
                context.moveTo(
                    width / 2 +
                        (player.pos.x -
                            focus.x +
                            player.radius * Math.cos(-Math.PI / 2)) *
                            fov,
                    height / 2 +
                        (player.pos.y -
                            focus.y +
                            player.radius * Math.sin(-Math.PI / 2)) *
                            fov
                );
                var numberOfSides = 4;
                if (player.shape == 1) {
        ...
        if (player.poison) {
            var poisoness =
                (player.poisonTimeLeft - player.poisonTime) /
                player.poisonTimeLeft;
            context.beginPath();
            context.fillStyle = "rgb(140, 1, 183," + poisoness + ")";
            context.arc(
                width / 2 + (player.pos.x - focus.x) * fov,
                height / 2 + (player.pos.y - focus.y) * fov,
                (player.radius + 0.5 / 32) * fov,
                0,
                Math.PI * 2,
                true
            );
        ...
        if (player.frozen) {
            var iceness =
                (player.frozenTimeLeft - player.frozenTime) /
                player.frozenTimeLeft;
            context.beginPath();
        ...
        if (player.burningTimer > 0) {
            context.beginPath();
            context.fillStyle =
                "rgb(0, 0, 0," + player.burningTimer / 1000 + ")";
            context.arc(
                width / 2 + (player.pos.x - focus.x) * fov,
                height / 2 + (player.pos.y - focus.y) * fov,
                player.radius * fov,
        ...
        //         context.drawImage(
        //             body,
        //             a - (5 * player.radius) / 3 + r,
        //             t - (5 * player.radius) / 3 + c,
        //             (10 * player.radius) / 3,
        //             (10 * player.radius) / 3
        //         );
        if (document.getElementById("body").value != "None")
        ...
        if (document.getElementById("wreath").value != "None")
            if (!player.reaperShade)
                context.drawImage(
                    hat,
                    width / 2 +
        ...
        context.closePath();
        context.beginPath();
        if (document.getElementById("wreath").value.endsWith("Crown"))
            if (!player.reaperShade)
                context.drawImage(
                    gem,
                    width / 2 +
        ...
        if (player.sweetToothConsumed) {
            context.fillStyle = "rgb(255, 43, 143)";
        }
        if (!player.reaperShade)
            context.fillRect(
                width / 2 + (player.pos.x - focus.x) * fov - (18 / 32) * fov,
                height / 2 +
                    (player.pos.y - focus.y) * fov -
                    player.radius * fov -
                    (8 / 32) * fov,
                ((36 / 32) * fov * player.energy) / player.maxEnergy,
                (7 / 32) * fov
            );
        context.fill();
        context.closePath();
        ...
        context.lineWidth = 1;
        if (player.sweetToothConsumed) {
            context.strokeStyle = "rgb(212, 0, 100)";
        }
        if (!player.reaperShade)
            context.strokeRect(
                width / 2 + (player.pos.x - focus.x) * fov - (18 / 32) * fov,
        ...
        context.fillStyle = "black";
        context.font =
            (12 / 32) * fov + "px Tahoma, Verdana, Segoe, sans-serif";
                ...
                if (entities[i][j].shatterTime > 0) {
                    ...
                    if (u < 250) {
                        ...
                        for (var o = 0; o < 8; o++) {
                            ...
                            context.fillStyle = entities[i][j].color;
                            context.arc(
                                midX + Math.cos(n) * f * s * fov,
                                midY + Math.sin(n) * f * s * fov,
                                (entities[i][j].radius / 3) * fov,
                                0,
                                2 * Math.PI,
                                !1
                            );
                        ...
                        for (var o = 0; o < 8; o++) {
                            ...
                            context.fillStyle = entities[i][j].color;
                            context.arc(
                                midX + Math.cos(n) * (s - h * s) * fov,
                                midY + Math.sin(n) * (s - h * s) * fov,
                                Math.min(
                                    entities[i][j].radius,
                                    Math.max(l, entities[i][j].radius * h)
                                ) * fov,
                                0,
                                2 * Math.PI,
                                !1
                    ...
                    if (entities[i][j].alpha) {
                        if (!entities[i][j].Harmless) {
                            context.globalAlpha = entities[i][j].alpha;
                        }
                    }
                    if (entities[i][j].color_change) {
                        const rgbColor = hexToRgb(entities[i][j].color);
                        rgbColor[0] =
                            parseInt(rgbColor[0]) + entities[i][j].color_change;
                        rgbColor[1] =
                            parseInt(rgbColor[1]) -
                            1.45 * entities[i][j].color_change;
                        rgbColor[2] =
                            parseInt(rgbColor[2]) -
                            1.3 * entities[i][j].color_change;
...
function renderMinimap(area, players, focus) {
  ...
  for(var i=0;i<this.entities.length;i++){
    ...
    for (var i in area.zones) {
        ...
        context.fillRect(
            (area.zones[i].pos.x - bound.x) * coef,
            height -
                minimapSize.y +
                (area.zones[i].pos.y - bound.y) * coef +
                yOff,
            area.zones[i].size.x * coef,
            area.zones[i].size.y * coef
        );
        context.closePath();
    }
    for (var i in players) {
        var newPos = new Vector(
            (players[i].pos.x - area.pos.x - bound.x) * coef,
            (players[i].pos.y - area.pos.y - bound.y) * coef
        );
        context.beginPath();
        context.fillStyle = players[i].color;
        context.arc(
            newPos.x,
            height - minimapSize.y + newPos.y + yOff,
            4,
            0,
            Math.PI * 2,
            true
...
function renderUI(area, players, focus) {
    ...
    if (!(game.players[0].magnet || game.players[0].flashlight)) {
        ...
        context.fillRect(width / 2 - 516 / 2, height - 85, 516 + 82, 85);
    }
    context.fill();
    context.closePath();
    ...
    if (!(game.players[0].magnet || game.players[0].flashlight)) {
        context.fillRect(width / 2 - 516 / 2, height - 100, 516, 15);
    } else {
        context.fillRect(width / 2 - 516 / 2, height - 100, 516 + 82, 15);
    }
    context.fill();
    context.closePath();
    ...
    if (!(game.players[0].magnet || game.players[0].flashlight)) {
        context.fillRect(
            width / 2 - 516 / 2,
            height - 100,
            (game.players[0].currentExperience /
                game.players[0].maxExperience) *
                516,
            15
        );
    } else {
    ...
    if (game.players[0].magnet || game.players[0].flashlight) {
        context.beginPath();
        if (game.players[0].magnet) {
            if (game.players[0].magnetDirection == "Down")
                context.drawImage(
                    magnetDown,
                    width / 2 + (516 - 132 + 82 + 82) / 2,
                    height - 68,
                    48,
                    48
                );
            else if (game.players[0].magnetDirection == "Up")
                context.drawImage(
                    magnetUp,
                    width / 2 + (516 - 132 + 82 + 82) / 2,
                    height - 68,
        ...
        } else if (game.players[0].flashlight) {
            context.drawImage(
                flashlight,
                width / 2 + (516 - 132 + 82 + 82) / 2,
                height - 68,
    ...
    if (game.players[0].hasAB) {
        ...
        for (var a = 0; a < 2; a++) {
            ...
            var abC =
                a == 1
                    ? players[0].firstAbilityCooldown
                    : players[0].secondAbilityCooldown;
            var abTC =
                a == 1
                    ? players[0].firstTotalCooldown
                    : players[0].secondTotalCooldown;
            var ab1ML =
                a == 1 ? players[0].ab1ML || false : players[0].ab2ML || false;
            if (!abL) {
                text = text3;
            }
            ...
            context.drawImage(
                ab,
                width / 2 - 516 / 2 + 105 + 41 + 246 + correct - 24,
                height - 85 - 3 + 17 + 44 - 17 - 24,
                48,
                48
            );
            context.fillText(
                text,
                width / 2 - 516 / 2 + 105 + 41 + 246 + correct,
                height - 85 + 17 / 2 + 44 - 17 + 24 + 17
            );
            context.closePath();
            if (!abL) {
                context.fillStyle = "rgba(0, 0, 0, 0.6)";
            ...
            for (var p = 0; p < 5; p++) {
                ...
                context.beginPath();
                var h = width / 2 - 516 / 2 + 105 + 41 + 246 + correct - 24 + 5;
                var f = h + 40;
                var y = height - 85 - 3 + 17 + 44 - 17 - 24 + 45 - 48 - 6;
                var b = ab1ML
                    ? h + (f - h) * (2 / (5 - 1))
                    : h + (f - h) * (p / (5 - 1));
    ...
    context.textAlign = "center";
    context.fillText(
        parseFloat(players[0].speed.toFixed(1)),
        width / 2 - 516 / 2 + 105 + 41,
        height - 85 + 17 + 44 - 17
    );
    context.closePath();
    ...
    context.textAlign = "center";
    context.fillText(
        Math.round(players[0].energy) + " / " + players[0].maxEnergy,
        width / 2 - 516 / 2 + 105 + 41 + 82,
        height - 85 + 17 + 44 - 17
    );
    context.closePath();
    ...
    context.textAlign = "center";
    context.fillText(
        Math.round(players[0].regen * 10) / 10,
        width / 2 - 516 / 2 + 105 + 41 + 164,
        height - 85 + 17 + 44 - 17
    );
    context.closePath();
...
function renderAssets(area, players, focus) {
    ...
    for (var i in area.assets) {
        ...
        for (var j = 0; j < zone.size.x; j++) {
            for (var k = 0; k < zone.size.y; k++) {
                ...
                context.drawImage(
                    tile_image,
                    Math.abs(posX) * 32,
                    Math.abs(posY) * 32 + zone.texture * 128,
                    32,
                    32,
                    width / 2 + (area.pos.x + zone.pos.x + j - focus.x) * fov,
                    height / 2 + (area.pos.y + zone.pos.y + k - focus.y) * fov,
                    fov,
                    fov
                );
                context.closePath();
        ...
        if (zone.type == 6) {
            ...
            context.drawImage(
                torch,
                width / 2 + (posX - focus.x) * fov,
                height / 2 + (posY - focus.y) * fov
            );
        } else if (zone.type == 7) {
            var posX = area.pos.x + zone.pos.x;
            var posY = area.pos.y + zone.pos.y;
